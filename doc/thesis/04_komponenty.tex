...
Klasa \textit{MonteCarloNode}\footnote{Diagramy na Rysunkach \ref{rys:umldiagram_node}-\ref{rys:sequencevisualise} zostały wygenerowane z użyciem oprogramowania \textit{PlantUML} na licencji \textit{GNU General Public License}.}, zgodnie z Rysunkiem \ref{rys:umldiagram_node}, reprezentuje wierzchołek w drzewie, więc przechowuje referencje do swojego rodzica i wierzchołków potomnych, aby zachować rekurencyjną strukturę drzewa. Ponadto zawiera wszystkie informacje związane z przebiegiem algorytmu UCT w polu \textit{details} oraz algorytmu Walkera w polu \textit{vis\textunderscore details}. \\
