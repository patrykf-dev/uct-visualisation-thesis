...
Poniższy rozdział ukazuje wszelkie założenia związane z projektem, a także przedstawia podział wykonanych zadań i harmonogram pracy.
...
Wymagania niefunkcjonalne opisane są w Tabeli \ref{tab:urps}. Zawiera ona założenia odnośnie jej używalności, niezawodności, wydajności i wsparcia.
...
Podział zaimplementowanych komponentów między autorów systemu opisuje Tabela \ref{tab:workdivision}, napisane rozdziały pracy -- Tabela \ref{tab:workdivision2}, a czasowy podział pracy -- harmonogram z Tabeli \ref{tab:schedule}. Tabela \ref{tab:schedule} nie opisuje zależności między kolejnymi fazami rozwoju projektu, wyznacza jedynie planowe terminy ich zakończenia. Tworząc czasowy podział pracy, autorzy rozłożyli pracę regularnie na cały zaplanowany czas tworzenia projektu. Ponadto, ze względu na skomplikowanie modułu odpowiedzialnego za wizualizację, wydzielono w nim trzy odrębne fazy rozwoju, wymienione poniżej.
