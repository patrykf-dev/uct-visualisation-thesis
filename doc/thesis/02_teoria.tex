...
W celu wyznaczenia układu wierzchołków drzewa na płaszczyźnie, który spełnia powyższe 3 założenia, skorzystano z usprawnionego algorytmu Walkera, którego złożoność czasowa jest liniowa względem liczby wierzchołków. Algorytm ten został zaimplementowany, a opisany jest dokładnie w \cite{impwalkers}, a grafiki użyte w tym podrozdziale również pochodzą z tego dokumentu. Algorytm jest dość obszerny, jednak jego główna idea sprowadza się do podwójnego przejścia po drzewie -- tak zwanego \textit{first walk} i \textit{second walk}.
\begin{enumerate}
