...
\section{Testy akceptacyjne} \label{sec:testy}
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\begin{itemize} 
			...
			\item Naciśnij przycisk \textit{Inspect tree}. \item Naciśnij przycisk \textit{Save as csv file}. 
			\item Naciśnij przycisk \textit{Save as binary file}. 
			\item Naciśnij przycisk \textit{Save as bitmap}. 
...
\section{Kontynuacja pracy} \label{sec:kontynuacja}
Aplikacja spełnia wszystkie początkowe wymagania określone w rozdziale \ref{chap:zal_pr}, jednak w przyszłości mogą zostać do niej dodane usprawnienia. W celu dalszej poprawy jakości i zwiększenia liczby możliwych opcji, obecny produkt można rozszerzyć o następujące funkcjonalności:
...
Jest kilka czynników wpływających negatywnie na szybkość wykonywania obliczeń w języku \textit{Python}. Przykładowymi czynnikami są fakt, że jest to język dynamicznie typowany, oraz to, że udostępnia relatywnie mało możliwości w zakresie zarządzania pamięcią. W celu przyspieszenia modułu odpowiedzialnego za grę w szachy, można by przepisać go w całości z języka \textit{Python} do języka takiego jak \textit{C++}.