...
\noindent Rysunek \ref{rys:statediagram} ukazuje diagram stanów aplikacji w przypadku rozgrywki w trybie \textit{człowiek kontra maszyna}. 
\begin{figure}[h]
	\centering
...
\noindent Zgodnie z diagramem, aplikacja po rozpoczęciu rozgrywki przechodzi do obszernego stanu \textit{Rozgrywka}, zawierającego cztery wewnętrzne stany. Będac w stanie \textit{Rozgrywka}, aplikacja może potencjalnie korzystać z każdego modułu aplikacji. \\

\noindent Istotna z punktu widzenia użytkownika jest możliwość serializowania wygenerowanego drzewa lub jego wizualizacja zaraz po ruchu wyznaczonym przez algorytm, co powoduje przejście aplikacji odpowiednio w stany \textit{Serializacja wygenerowanego drzewa} oraz \textit{Wyświetlanie wygenerowanego drzewa}.
\clearpage

\subsection{Diagram sekwencji rozgrywki}
Rysunek \ref{rys:sequencegame} ukazuje diagram sekwencji rozgrywki w trybie \textit{człowiek kontra maszyna}.
\begin{figure}[h]
	\centering
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\textwidth]{serialize_sequence}
...
\noindent Jest to diagram dla ustawienia \textit{człowiek kontra maszyna}, jednak w przypadku \textit{maszyna kontra maszyna} istnieje taka sama funkcjonalność i diagram byłby analogiczny.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\textwidth]{visualization_sequence}
	\caption{Diagram sekwencji wizualizacji}
	\label{rys:sequencevisualise}
\end{figure}

\noindent Ważne jest, że użytkownik może uruchomić wizualizację z poziomu rozgrywki tuż po wygenerowaniu nowego drzewa przez algorytm lub już na etapie menu głównego. Gdy żądanie jest z poziomu rozgrywki, komponent \textit{Aplikacja główna} komunikuje się z komponentem \textit{Wizualizacja}, który generuje aktualne drzewo i pokazuje je użytkownikowi w nowym oknie. \\
