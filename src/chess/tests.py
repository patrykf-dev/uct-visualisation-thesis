...
class TestChess(unittest.TestCase):
    ...
    def test_en_passant_capture_black(self):
        ...
        self.make_moves_from_queue(moves)
        self.assertIsNone(Figure.get_figure(self.game.game_manager.board.figures, (3, 6)))
    ...
    def test_castle_short_execution_both_colors(self):
        ...
        self.make_moves_from_queue(moves)
        self.assertIsNotNone(Figure.get_figure(self.game.game_manager.board.figures, (0, 5)))
        self.assertIsNotNone(Figure.get_figure(self.game.game_manager.board.figures, (0, 6)))
        self.assertIsNotNone(Figure.get_figure(self.game.game_manager.board.figures, (7, 5)))
        self.assertIsNotNone(Figure.get_figure(self.game.game_manager.board.figures, (7, 6)))
        figure = Figure.get_figure(self.game.game_manager.board.figures, (0, 5))
        self.assertEqual(figure.figure_type, FigureType.ROOK)
        figure = Figure.get_figure(self.game.game_manager.board.figures, (0, 6))
        self.assertEqual(figure.figure_type, FigureType.KING)
        figure = Figure.get_figure(self.game.game_manager.board.figures, (7, 5))
        self.assertEqual(figure.figure_type, FigureType.ROOK)
        figure = Figure.get_figure(self.game.game_manager.board.figures, (7, 6))
        self.assertEqual(figure.figure_type, FigureType.KING)
    ...
    def test_castle_long_execution_both_colors(self):
        ...
        self.make_moves_from_queue(moves)
        self.assertIsNotNone(Figure.get_figure(self.game.game_manager.board.figures, (0, 2)))
        self.assertIsNotNone(Figure.get_figure(self.game.game_manager.board.figures, (0, 3)))
        self.assertIsNotNone(Figure.get_figure(self.game.game_manager.board.figures, (7, 2)))
        self.assertIsNotNone(Figure.get_figure(self.game.game_manager.board.figures, (7, 3)))
        figure = Figure.get_figure(self.game.game_manager.board.figures, (0, 3))
        self.assertEqual(figure.figure_type, FigureType.ROOK)
        figure = Figure.get_figure(self.game.game_manager.board.figures, (0, 2))
        self.assertEqual(figure.figure_type, FigureType.KING)
        figure = Figure.get_figure(self.game.game_manager.board.figures, (7, 3))
        self.assertEqual(figure.figure_type, FigureType.ROOK)
        figure = Figure.get_figure(self.game.game_manager.board.figures, (7, 2))
        self.assertEqual(figure.figure_type, FigureType.KING)
    ...
    def test_king_cant_step_back_when_check(self):
        ...
        self.make_moves_from_queue(moves)
        self.assertNotIn((0, 4), [move.position_to for move in self.game.game_manager.board.possible_moves])
    ...
    def test_checkmate_ends_game(self):
        ...
        moves = [Move((0, 6), (7, 6))]
        self.make_moves_from_queue(moves)
        self.assertFalse(ChessUtils.is_there_any_possible_move(self.game.game_manager.board))
    ...
    def test_checkmate_can_be_prevented_by_capture(self):
        ...
        moves = [Move((0, 6), (7, 6))]
        self.make_moves_from_queue(moves)
        self.assertTrue(ChessUtils.is_there_any_possible_move(self.game.game_manager.board))
    ...
    def test_are_moves_reduced_during_check(self):
        ...
        self.assertIn((3, 3), [move.position_to for move in self.game.game_manager.board.possible_moves])
        self.assertNotIn((3, 2), [move.position_to for move in self.game.game_manager.board.possible_moves])

        select = [Move((0, 6))]
        self.make_moves_from_queue(select)
        self.assertIn((3, 3), [move.position_to for move in self.game.game_manager.board.possible_moves])
        self.assertNotIn((1, 5), [move.position_to for move in self.game.game_manager.board.possible_moves])

        select = [Move((2, 1))]
        self.make_moves_from_queue(select)
        self.assertIn((3, 3), [move.position_to for move in self.game.game_manager.board.possible_moves])
        self.assertNotIn((3, 1), [move.position_to for move in self.game.game_manager.board.possible_moves])

        select = [Move((0, 0))]
        self.make_moves_from_queue(select)
        self.assertIn((3, 3), [move.position_to for move in self.game.game_manager.board.possible_moves])
        self.assertNotIn((1, 1), [move.position_to for move in self.game.game_manager.board.possible_moves])

        select = [Move((7, 4))]
        self.make_moves_from_queue(select)
        self.assertIn((4, 4), [move.position_to for move in self.game.game_manager.board.possible_moves])
        self.assertNotIn((7, 0), [move.position_to for move in self.game.game_manager.board.possible_moves])

        select = [Move((0, 3))]
        self.make_moves_from_queue(select)
        self.assertIn((3, 3), [move.position_to for move in self.game.game_manager.board.possible_moves])
        self.assertNotIn((0, 1), [move.position_to for move in self.game.game_manager.board.possible_moves])

        select = [Move((5, 5))]
        self.make_moves_from_queue(select)
        self.assertIn((5, 4), [move.position_to for move in self.game.game_manager.board.possible_moves])
        self.assertIn((4, 5), [move.position_to for move in self.game.game_manager.board.possible_moves])
        self.assertNotIn((4, 4), [move.position_to for move in self.game.game_manager.board.possible_moves])
        self.assertNotIn((6, 6), [move.position_to for move in self.game.game_manager.board.possible_moves])
    ...
    def test_exposure_of_king_should_not_be_possible(self):
        ...
        moves = [Move((2, 2))]
        self.make_moves_from_queue(moves)
        self.assertNotIn((2, 3), [move.position_to for move in self.game.game_manager.board.possible_moves])
    ...
    #         self.make_moves_from_queue(moves)
    #     figure = Figure.get_figure(self.game.game_manager.board.figures, (7, 4))
    ...
    def test_prevent_from_short_castling_when_fields_on_the_way_are_attacked(self):
        ...
        figures = self.game.game_manager.board.figures = ChessFiguresCollection(figures)
        figures.get_king(Color.WHITE).update_check_mask(figures)
        moves = [Move((0, 4))]
        self.make_moves_from_queue(moves)
        self.assertNotIn((0, 6), [move.position_to for move in self.game.game_manager.board.possible_moves])

        moves = [Move((4, 5), (5, 5)), Move((7, 4))]
        self.make_moves_from_queue(moves)
        self.assertNotIn((7, 6), [move.position_to for move in self.game.game_manager.board.possible_moves])
    ...
    def test_prevent_from_long_castling_when_fields_on_the_way_are_attacked(self):
        ...
        moves = [Move((0, 4))]
        self.make_moves_from_queue(moves)
        self.assertNotIn((0, 2), [move.position_to for move in self.game.game_manager.board.possible_moves])

        moves = [Move((4, 3), (5, 3)), Move((7, 4))]
        self.make_moves_from_queue(moves)
        self.assertNotIn((7, 2), [move.position_to for move in self.game.game_manager.board.possible_moves])
    ...
    def test_checkmate_black(self):
        ...
        self.make_moves_from_queue(moves)
        self.assertEqual(self.game.game_manager.board.game_status, GameStatus.CHECKMATE_BLACK)

    def test_checkmate_white(self):
        moves = [Move((1, 4), (2, 4)), Move((6, 5), (4, 5)), Move((1, 5), (3, 5)), Move((6, 6), (4, 6)),
                 Move((0, 3), (4, 7))]
        self.make_moves_from_queue(moves)
        self.assertEqual(self.game.game_manager.board.game_status, GameStatus.CHECKMATE_WHITE)
    ...
    def test_stalemate_black(self):
        ...
        moves = [Move((4, 4), (5, 5))]
        self.make_moves_from_queue(moves)
        self.assertEqual(self.game.game_manager.board.game_status, GameStatus.STALEMATE)
    ...
    def test_is_draw_when_two_kings(self):
        ...
        moves = [Move((0, 0), (1, 1))]
        self.make_moves_from_queue(moves)
        self.assertEqual(self.game.game_manager.board.game_status, GameStatus.DRAW)
    ...
    def test_is_not_draw_when_king_rook(self):
        ...
        moves = [Move((0, 0), (1, 1))]
        self.make_moves_from_queue(moves)
        self.assertEqual(self.game.game_manager.board.game_status, GameStatus.IN_PROGRESS)
