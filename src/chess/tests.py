...
class TestChess(unittest.TestCase):
    ...
    def mock_process_input(self, event_queue, sleep_time=0.0):
        for event in event_queue:
            if event.type == QUIT:
                ...
                self.game.decide_move_action(grid_pos[::-1])
                self.game.draw_board()
                self.game.draw_moves()
                self.game.check_and_set_game_status()
                if sleep_time:
    ...
    def test_en_passant_capture_possibility_white(self):
        ...
        self.assertIn((5, 1), [move.position for move in self.game.possible_moves])
        moves = [Move((1, 2), (3, 2)), Move((4, 7), (3, 7)),
                 Move((4, 0))]
        self.make_moves_from_queue(moves)
        self.assertNotIn((5, 1), [move.position for move in self.game.possible_moves])
    ...
    def test_en_passant_capture_possibility_black(self):
        ...
        self.make_moves_from_queue(moves)
        self.assertIn((2, 6), [move.position for move in self.game.possible_moves])

        moves = [Move((6, 0), (5, 0)), Move((1, 1), (3, 1)),
                 Move((3, 7))]
        self.make_moves_from_queue(moves)
        self.assertNotIn((2, 6), [move.position for move in self.game.possible_moves])
    ...
    def test_en_passant_capture_white(self):
        moves = [Move((1, 0), (3, 0)), Move((6, 7), (4, 7)),
                 ...
                 Move((4, 0), (5, 1))]
        self.make_moves_from_queue(moves)
        self.assertIsNone(Figure.get_figure(self.game.figures, (4, 1)))
    ...
    def test_en_passant_capture_black(self):
        ...
        self.make_moves_from_queue(moves)
        self.assertIsNone(Figure.get_figure(self.game.figures, (3, 6)))
    ...
    def test_castle_short_execution_both_colors(self):
        ...
        self.make_moves_from_queue(moves)
        self.assertIsNotNone(Figure.get_figure(self.game.figures, (0, 5)))
        self.assertIsNotNone(Figure.get_figure(self.game.figures, (0, 6)))
        self.assertIsNotNone(Figure.get_figure(self.game.figures, (7, 5)))
        self.assertIsNotNone(Figure.get_figure(self.game.figures, (7, 6)))
        figure = Figure.get_figure(self.game.figures, (0, 5))
        self.assertEqual(figure.figure_type, FigureType.ROOK)
        figure = Figure.get_figure(self.game.figures, (0, 6))
        self.assertEqual(figure.figure_type, FigureType.KING)
        figure = Figure.get_figure(self.game.figures, (7, 5))
        self.assertEqual(figure.figure_type, FigureType.ROOK)
        figure = Figure.get_figure(self.game.figures, (7, 6))
        self.assertEqual(figure.figure_type, FigureType.KING)
    ...
    def test_castle_long_execution_both_colors(self):
        ...
        self.make_moves_from_queue(moves)
        self.assertIsNotNone(Figure.get_figure(self.game.figures, (0, 2)))
        self.assertIsNotNone(Figure.get_figure(self.game.figures, (0, 3)))
        self.assertIsNotNone(Figure.get_figure(self.game.figures, (7, 2)))
        self.assertIsNotNone(Figure.get_figure(self.game.figures, (7, 3)))
        figure = Figure.get_figure(self.game.figures, (0, 3))
        self.assertEqual(figure.figure_type, FigureType.ROOK)
        figure = Figure.get_figure(self.game.figures, (0, 2))
        self.assertEqual(figure.figure_type, FigureType.KING)
        figure = Figure.get_figure(self.game.figures, (7, 3))
        self.assertEqual(figure.figure_type, FigureType.ROOK)
        figure = Figure.get_figure(self.game.figures, (7, 2))
        self.assertEqual(figure.figure_type, FigureType.KING)
    ...
    def test_king_cant_step_back_when_check(self):
        ...
        self.make_moves_from_queue(moves)
        self.assertNotIn((0, 4), [move.position for move in self.game.possible_moves])

    def test_checkmate_ends_game(self):
        self.game.figures = [King(Color.BLACK, (7, 0)), Rook(Color.WHITE, (6, 7)), Rook(Color.WHITE, (0, 6)),
                             King(Color.WHITE, (0, 0))]
        moves = [Move((0, 6), (7, 6))]
        self.make_moves_from_queue(moves)
        self.assertFalse(self.game.is_there_any_possible_move())
    ...
    def test_checkmate_can_be_prevented_by_capture(self):
        ...
        moves = [Move((0, 6), (7, 6))]
        self.make_moves_from_queue(moves)
        self.assertTrue(self.game.is_there_any_possible_move())

    def test_are_moves_reduced_during_check(self):
        self.game.figures = [King(Color.BLACK, (5, 5)), Rook(Color.BLACK, (0, 3)), Rook(Color.BLACK, (7, 4)),
        ...
        self.assertIn((3, 3), [move.position for move in self.game.possible_moves])
        self.assertNotIn((3, 2), [move.position for move in self.game.possible_moves])

        select = [Move((0, 6))]
        self.make_moves_from_queue(select)
        self.assertIn((3, 3), [move.position for move in self.game.possible_moves])
        self.assertNotIn((1, 5), [move.position for move in self.game.possible_moves])

        select = [Move((2, 1))]
        self.make_moves_from_queue(select)
        self.assertIn((3, 3), [move.position for move in self.game.possible_moves])
        self.assertNotIn((3, 1), [move.position for move in self.game.possible_moves])

        select = [Move((0, 0))]
        self.make_moves_from_queue(select)
        self.assertIn((3, 3), [move.position for move in self.game.possible_moves])
        self.assertNotIn((1, 1), [move.position for move in self.game.possible_moves])

        select = [Move((7, 4))]
        self.make_moves_from_queue(select)
        self.assertIn((4, 4), [move.position for move in self.game.possible_moves])
        self.assertNotIn((7, 0), [move.position for move in self.game.possible_moves])

        select = [Move((0, 3))]
        self.make_moves_from_queue(select)
        self.assertIn((3, 3), [move.position for move in self.game.possible_moves])
        self.assertNotIn((0, 1), [move.position for move in self.game.possible_moves])

        select = [Move((5, 5))]
        self.make_moves_from_queue(select)
        self.assertIn((5, 4), [move.position for move in self.game.possible_moves])
        self.assertIn((4, 5), [move.position for move in self.game.possible_moves])
        self.assertNotIn((4, 4), [move.position for move in self.game.possible_moves])
        self.assertNotIn((6, 6), [move.position for move in self.game.possible_moves])

    def test_exposure_of_king_should_not_be_possible(self):
        self.game.figures = [King(Color.WHITE, (4, 2)), Rook(Color.WHITE, (2, 2)), Rook(Color.BLACK, (0, 2)),
                             King(Color.BLACK, (7, 7))]
        moves = [Move((2, 2))]
        self.make_moves_from_queue(moves)
        self.assertNotIn((2, 3), [move.position for move in self.game.possible_moves])

    def test_promotion(self):
        self.game.figures = [King(Color.WHITE, (1, 0)), Pawn(Color.WHITE, (6, 4)), King(Color.BLACK, (6, 7)),
                             Pawn(Color.BLACK, (1, 3))]
        moves = [Move((6, 4), (7, 4))]
        with patch('builtins.input', side_effect="q"):
            self.make_moves_from_queue(moves)
        figure = Figure.get_figure(self.game.figures, (7, 4))
    ...
    def test_prevent_from_short_castling_when_fields_on_the_way_are_attacked(self):
        self.game.figures = [King(Color.WHITE, (0, 4)), Rook(Color.BLACK, (3, 5)), King(Color.BLACK, (7, 4)),
                             Rook(Color.WHITE, (4, 5)), Rook(Color.WHITE, (0, 7)), Rook(Color.BLACK, (7, 7))]
        moves = [Move((0, 4))]
        self.make_moves_from_queue(moves)
        self.assertNotIn((0, 6), [move.position for move in self.game.possible_moves])

        moves = [Move((4, 5), (5, 5)), Move((7, 4))]
        self.make_moves_from_queue(moves)
        self.assertNotIn((7, 6), [move.position for move in self.game.possible_moves])

    def test_prevent_from_long_castling_when_fields_on_the_way_are_attacked(self):
        self.game.figures = [King(Color.WHITE, (0, 4)), Rook(Color.BLACK, (3, 3)), King(Color.BLACK, (7, 4)),
                             Rook(Color.WHITE, (4, 3)), Rook(Color.WHITE, (0, 0)), Rook(Color.BLACK, (7, 0))]
        moves = [Move((0, 4))]
        self.make_moves_from_queue(moves)
        self.assertNotIn((0, 2), [move.position for move in self.game.possible_moves])

        moves = [Move((4, 3), (5, 3)), Move((7, 4))]
        self.make_moves_from_queue(moves)
        self.assertNotIn((7, 2), [move.position for move in self.game.possible_moves])

    def test_prevent_from_short_castling_when_the_last_field_on_the_way_is_attacked(self):
        self.game.figures = [King(Color.WHITE, (0, 4)), Knight(Color.BLACK, (2, 7)), King(Color.BLACK, (7, 4)),
    ...
    def test_prevent_form_long_castling_when_the_last_field_on_the_way_is_attacked(self):
        self.game.figures = [King(Color.WHITE, (0, 4)), Knight(Color.BLACK, (2, 1)), King(Color.BLACK, (7, 4)),
    ...
    def test_prevent_from_castling_when_check(self):
        self.game.figures = [King(Color.WHITE, (0, 4)), Rook(Color.BLACK, (3, 4)), King(Color.BLACK, (7, 4)),
    ...
    def test_prevent_short_castling_around_the_other_king(self):
        self.game.figures = [King(Color.WHITE, (0, 4)), King(Color.BLACK, (1, 6)), Rook(Color.WHITE, (0, 7))]
        moves = [Move((0, 4))]
        self.make_moves_from_queue(moves)
        self.assertNotIn((0, 6), [move.position for move in self.game.possible_moves])

    def test_prevent_long_castling_around_the_other_king(self):
        self.game.figures = [King(Color.WHITE, (0, 4)), King(Color.BLACK, (1, 2)), Rook(Color.WHITE, (0, 0)),
    ...
    def test_checkmate_black(self):
        ...
        self.make_moves_from_queue(moves)
        self.assertEqual(self.game.game_status, GameStatus.CHECKMATE_BLACK)

    def test_checkmate_white(self):
        moves = [Move((1, 4), (2, 4)), Move((6, 5), (4, 5)), Move((1, 5), (3, 5)), Move((6, 6), (4, 6)),
                 Move((0, 3), (4, 7))]
        self.make_moves_from_queue(moves)
        self.assertEqual(self.game.game_status, GameStatus.CHECKMATE_WHITE)

    def test_stalemate_black(self):
        self.game.figures = [King(Color.WHITE, (4, 4)), King(Color.BLACK, (7, 5)), Pawn(Color.WHITE, (6, 5))]
        moves = [Move((4, 4), (5, 5))]
        self.make_moves_from_queue(moves)
        self.assertEqual(self.game.game_status, GameStatus.STALEMATE)

    def test_stalemate_white(self):
        self.game.figures = [King(Color.BLACK, (3, 3)), King(Color.WHITE, (1, 1)), Pawn(Color.BLACK, (1, 2))]
    ...
    def test_is_draw_when_two_kings(self):
        self.game.figures = [King(Color.WHITE, (0, 0)), King(Color.BLACK, (7, 7)), Rook(Color.BLACK, (1, 1))]
        moves = [Move((0, 0), (1, 1))]
        self.make_moves_from_queue(moves)
    ...
    def test_fifty_move_rule(self):
        self.game.figures = [King(Color.WHITE, (0, 0)), King(Color.BLACK, (6, 0)), Pawn(Color.WHITE, (3, 3))]
        ...
        moves = [Move((0, 2), (0, 3))]
        self.make_moves_from_queue(moves)
        self.assertEqual(self.game.game_status, GameStatus.FIFTY_MOVE_RULE)
