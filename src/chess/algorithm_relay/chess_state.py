...
class ChessState(BaseGameState):
    def __init__(self, board):
        super().__init__()
        self.board = board
        self.current_player = 1 if board.move_color == Color.WHITE else 2
    ...
    def perform_random_move(self):
        ...
        print(f"Player {move.player} performing move from {move.position_from} to {move.position_to}, figures left {len(self.board.figures)}")
